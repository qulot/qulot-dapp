<template>
    <Dropdown title="Network select" :items="items" class="p-2 rounded gap-2" @item-click="onChange">
...
const items = computed(() => networks.map((network) => ({
    key: network.key,
...
const selected = computed(() => items.value.find(network => network.key === networkSetting.value))
