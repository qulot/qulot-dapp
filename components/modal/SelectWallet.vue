      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 h-full">
        ...
        <div class="order-2 flex flex-col px-6 pb-4 pt-4 lg:pt-[56px] gap-y-6">
          ...
          <div class="grid grid-cols-4 items-start justify-start gap-6">
            <WalletItem
              v-for="item in availableWallets"
              :key="item.id"
              :icon="item.icon"
              :name="item.name"
              @click="onClick(item)"
...
const openWalletWithDeepLink = async (wallet: string) => {
  const deepLink = generateWalletDeepLink(wallet)
  if (deepLink) {
    try {
...
const onClick = async (wallet: any) => {
  ...
  if (!connectorIsReady(wallet.id) && isMobile()) {
    await openWalletWithDeepLink(wallet.id)
  } else {
